...
AGLT_L_con<-function(AGLTtable_L){
  ...
  assign("aud_L_con_meanRT", mean(vec,na.rm=TRUE), envir=globalenv())
  }
...
AGLT_L_incon<-function(AGLTtable_L){
  ...
  for (i in 1:80) {
    ...
    assign("aud_L_incon_meanRT", mean(vec,na.rm=TRUE), envir=globalenv())
  }
...
AGLT_G_con<-function(AGLTtable_G){
  ...
  for (i in 1:80) {
    ...
    assign("aud_G_con_meanRT", mean(vec,na.rm=TRUE), envir=globalenv())
  }
...
AGLT_G_incon<-function(AGLTtable_G){
  ...
  for (i in 1:80) {
    ...
    assign("aud_G_incon_meanRT", mean(vec,na.rm=TRUE), envir=globalenv())
  }
